...
const Home = props => {
  ...
  return (
    <div>
      ...
      <ul style={{ position: 'relative', top: '20px', listStyle: 'none' }}>
        {data.sessions.map(session => (
          ...
          <li key={session.id}>
            ...
            <SessionItem
              ...
              onClick={() => {
                setBookingButton(
                  bookingButton === session.id ? null : session.id
                );
              }}
              style={{
                display: 'inline-flex',
                ...
                margin:
                  ...
                  !initBookedSessions.includes(session.id)
                    ? '0'
                    : '0 0 10px 0',
                backgroundColor: !!initBookedSessions.includes(session.id)
                  ? '#f26640'
                  : !(session.max_slots - session.slots_booked)
                  ? '#dc3545'
                  : '#e1ded1'
            ...
            >
              <div>
                Date: {session.session_datetime.slice(0, 10)} Time:{' '}
                {session.session_datetime.slice(11)} has{' '}
                {session.max_slots - session.slots_booked} slots available
                {initBookedSessions.includes(session.id) ? ' Booked!' : null}
              </div>
            </SessionItem>
